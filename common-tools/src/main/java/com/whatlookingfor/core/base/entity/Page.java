...
import com.whatlookingfor.common.utils.CookieUtils;
...
public class Page<T> {
	...
	/**
	 * 标准查询有效， 实例： updatedate desc, name asc,排序规则
	 */
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
		...
		if (StringUtils.isNumeric(no)) {
			CookieUtils.setCookieValue(response, "pageNo", no);
			this.setPageNo(Integer.parseInt(no));
		} else if (request.getParameter("repage") != null) {
			no = CookieUtils.getCookieValue(request, "pageNo");
			if (StringUtils.isNumeric(no)) {
				this.setPageNo(Integer.parseInt(no));
			}
		}
		...
		if (StringUtils.isNumeric(size)) {
			CookieUtils.setCookieValue(response, "pageSize", size);
			this.setPageSize(Integer.parseInt(size));
		} else if (request.getParameter("repage") != null) {
			no = CookieUtils.getCookieValue(request, "pageSize");
			if (StringUtils.isNumeric(size)) {
				this.setPageSize(Integer.parseInt(size));
			}
